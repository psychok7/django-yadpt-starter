...
# worker = 2 * CPUs + 1.
gunicorn {{ project_name }}.wsgi:application -w 5 --worker-class 'gevent' --log-level debug --access-logfile logs/gunicorn/access.log --log-file logs/gunicorn/gunicorn.log -e DJANGO_SETTINGS_MODULE={{ project_name }}.settings.production -b :8000
